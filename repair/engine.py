...
1. Reparación de Vinculaciones (containerId <-> boundElements)
2. Reparación de Elementos de Texto (originalText, width, height, baseline, lineHeight)
3. Reparación de Confinamiento de Frames (coordenadas relativas -> absolutas y auto-expansión)
4. Reparación de Acentos Visuales (Regla del Acento Único)
"""
...
class RepairEngine:
    ...
    def auto_repair(scene_data: Dict[str, Any],
        ...
        # 3. Reparación de Confinamiento y Posicionamiento de Frames
        frame_repairs = repair_frame_containment(scene_data)
        all_repairs.extend(frame_repairs)

        # 4. Reparación de Acentos Visuales (Hero único)
