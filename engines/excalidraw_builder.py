"""
Sketion Excalidraw Builder Core
...
class ExcalidrawScene:
    ...
    def add_arrow(self, x1: float, y1: float, x2: float, y2: float,
                  ...
                  label: Optional[str] = None, frame_id: Optional[str] = None) -> Dict[str, Any]:
        """Crea una flecha con coordenadas relativas."""
        dx = x2 - x1
        dy = y2 - y1
    ...
    def add_sticky_label(self, x: float, y: float, text: str,
                         ...
                         font_size: int = 16, angle_deg: float = -2.0,
    ...
    def add_capture_slot(self, x: float, y: float, w: float, h: float,
                         label: str = "PEGAR CAPTURA / LOGO", frame_id: Optional[str] = None):
        """Crea un hueco punteado para screenshots."""
        return self.add_bound_card(x, y, w, h, label, bg="#F1F5F9", stroke="#94A3B8",
                                  text_color="#64748B", font_size=12, font_family=3,
                                  stroke_style="dashed", frame_id=frame_id)
