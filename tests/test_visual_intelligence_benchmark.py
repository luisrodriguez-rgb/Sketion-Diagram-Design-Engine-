...
1. 150+ Íconos Semánticos Vectoriales (100% Cero Emojis)
2. Reconocimiento de Marcas y Tecnologías (Brand Registry)
3. Formas Morfológicas Especializadas (Funnels, KPIs, Cilindros, Tuberías, Barreras, Pastillas)
4. Layout Zoned & Anti-Collision Engine: Zonas de Disposición Vertical Estricta
5. Ajuste Dinámico y Ceñido de Frames (Auto-Fit Frames)
"""
...
AUDIENCES = [
    VisualAudienceLanguage("CEO", "CEO & Inversionista", "KPIs & Funnel", "Métricas Financieras y Retorno de Negocio", "#059669", 1820.0, 560.0),
    VisualAudienceLanguage("TECH", "Senior Cloud Engineer", "Cilindros & Kafka Pipes", "Topología de Servicios y Resiliencia", "#2563EB", 1520.0, 560.0),
    VisualAudienceLanguage("AUDITOR", "Auditor de Riesgo & SOC2", "Bóvedas & Barreras WAF", "Control de Cumplimiento y Cifrado", "#D93829", 1440.0, 560.0),
    VisualAudienceLanguage("OPERATIONS", "Gerente de Operaciones", "Actores & Estados Vivos", "SLA Operativo y Conciliación Diaria", "#D97706", 1320.0, 560.0)
]
...
def run_visual_intelligence_benchmark():
    ...
    place_reset(max_row_w=3800, gap=120)
    ...
    for idx, aud in enumerate(AUDIENCES):
        ...
        # Renderizado semántico especializado por audiencia con disposición en zonas limpias
        if aud.code == "CEO":
            # 1. Funnel de Negocio a la izquierda (520px)
            fun_steps = [
            ...
            ch_x = kpi_x + 530.0
            scene.add_quad_card(ch_x, fy + 100.0, 560.0, 135.0, "Canales de Adquisición B2B & Retail", sublabel="Web SPA, SDK Móvil nativo, POS Terminal y WhatsApp Pay.\nIntegración unificada bajo un único contrato de cobro.", badge="ADQUISICIÓN", icon="users", frame_id=fid)
            scene.add_quad_card(ch_x, fy + 255.0, 560.0, 135.0, "Redes Financieras Internacionales", sublabel="Visa / Mastercard Direct Switch, ACH, SEPA Instant, PIX y PSE.\nCobertura multimoneda (USD, EUR, BRL, COP).", badge="GLOBAL RAILS", icon="card", frame_id=fid)
            ...
            srv_x = fx + 390.0
            scene.add_quad_card(srv_x, fy + 100.0, 390.0, 135.0, "Kong API Gateway Cluster", sublabel="Ruteo dinámico de microservicios y JWT auth.\nInyección de Correlation ID distribuido.", badge="GATEWAY", icon="server", frame_id=fid)
            scene.add_quad_card(srv_x, fy + 255.0, 390.0, 135.0, "Payment Orchestrator Core", sublabel="Máquina de estados distribuida (Patrón Saga).\nIdempotencia atómica SETNX con TTL 24h.", badge="HERO CORE", icon="server", is_hero=True, frame_id=fid)

            # 3. Tubería Kafka & Cilindros de DB
            pipe_x = srv_x + 410.0
            scene.add_streaming_pipe(pipe_x, fy + 100.0, 560.0, 95.0, "Apache Kafka Distributed Event Stream", ["payment.created", "payment.settled", "payment.failed"], badge="KAFKA CLUSTER", frame_id=fid)

            cyl_w = 270.0
            scene.add_database_cylinder(pipe_x, fy + 215.0, cyl_w, 175.0, "Aurora PostgreSQL", "Primary DB SERIALIZABLE\nRead Replicas con Lag <50ms", badge="ACID DB", is_hero=True, frame_id=fid)
            scene.add_database_cylinder(pipe_x + cyl_w + 20.0, fy + 215.0, cyl_w, 175.0, "ClickHouse OLAP", "Motor Columnar Real-Time\nConsultas masivas en ms", badge="OLAP SINK", frame_id=fid)
            ...
            scene.add_quad_card(fx + 50.0, fy + 100.0, 390.0, 140.0, "PCI-DSS Tokenizer Vault", sublabel="Aislamiento estricto de números PAN y CVV.\nTokenización irreversible con módulo HSM.\nCero exposición en logs de backend.", badge="PCI LEVEL 1", icon="lock", is_hero=True, frame_id=fid)

            # 2. Pistas de Auditoría WORM
            scene.add_database_cylinder(fx + 50.0, fy + 260.0, 390.0, 130.0, "MinIO S3 Immutable Vault", "Almacenamiento de objetos con políticas WORM.\nEvidencia inmutable para auditorías externas.", badge="WORM COMPLIANCE", frame_id=fid)

            # 3. Compliance Engines & Fiscalidad
            comp_x = fx + 460.0
            scene.add_quad_card(comp_x, fy + 100.0, 390.0, 140.0, "AML Anti-Money Laundering", sublabel="Chequeo en tiempo real contra listas restrictivas.\nOFAC, ONU, PEPs y scoring de riesgo dinámico.", badge="AML / KYC", icon="shield", frame_id=fid)
            scene.add_quad_card(comp_x, fy + 260.0, 390.0, 130.0, "DIAN Electronic Tax Reporter", sublabel="Generación automatizada de facturación electrónica.\nSincronización directa con entidades tributarias.", badge="DIAN FISCAL", icon="file", frame_id=fid)

            # 4. SOC2 Collector
            scene.add_quad_card(comp_x + 410.0, fy + 100.0, 420.0, 290.0, "SOC 2 Evidence Collector", sublabel="Telemetría continua de cumplimiento:\n• Control de acceso RBAC y rotación de claves.\n• Cifrado Zero-Trust en reposo (AES-256) y tránsito (TLS 1.3).\n• Trazabilidad total de despliegues y cambios en prod.", badge="SOC 2 CERTIFIED", icon="lock", frame_id=fid)
            ...
            scene.add_actor_node(fx + 50.0, fy + 100.0, 260.0, 58.0, "POS Smart Terminal", "ACTIVE (Retail 24/7)", icon="card", frame_id=fid)
            scene.add_actor_node(fx + 50.0, fy + 175.0, 260.0, 58.0, "WhatsApp Checkout", "ACTIVE (API v2)", icon="users", frame_id=fid)
            scene.add_actor_node(fx + 50.0, fy + 250.0, 260.0, 58.0, "Customer Support Desk", "ONLINE (Nivel 2)", icon="users", frame_id=fid)
            scene.add_actor_node(fx + 50.0, fy + 325.0, 260.0, 58.0, "Chargeback Handler", "PENDING (3 casos)", icon="file", frame_id=fid)

            # 2. Worker de Conciliación Hero
            ops_x = fx + 330.0
            scene.add_feature_card(ops_x, fy + 100.0, 440.0, 165.0, "Nightly Reconciliation Worker", [
            ...
            scene.add_quad_card(ops_x, fy + 280.0, 440.0, 110.0, "Grafana Real-Time NOC Wall", sublabel="Mural de operaciones con semáforo de estado de pasarelas.\nEscalamiento automático PagerDuty ante caídas de SLA.", badge="NOC MONITOR", icon="laptop", frame_id=fid)

            # 4. Callout Operativo
            scene.add_quad_card(ops_x + 460.0, fy + 100.0, 360.0, 290.0, "Protocolos de Falla & Runbooks", sublabel="• Timeout Pasarela (>8s) -> Apertura de Circuit Breaker.\n• Fallback POS Offline -> Criptofirma local.\n• Retry Webhooks -> Backoff exponencial con Jitter.", badge="RUNBOOK SRE", icon="alert", frame_id=fid)
